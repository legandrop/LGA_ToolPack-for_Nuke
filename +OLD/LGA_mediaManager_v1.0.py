    ...
    def go_to_read(self):
        ...
        if selected_items:
            ...
            if read_node:
                ...
                #print(f"No se encontro el nodo Read: {next_read_node_name}")
                pass
...
##### Borrado:
    def delete_selected(self):
        ...
        if selected_items:
            file_path = selected_items[0].text()  # Obtiene el file_path de la fila seleccionada
            status = selected_items[2].text()  # Obtiene el estado del archivo seleccionado
            
            # Verifica si el estado es 'Offline'
            if status == "Offline":
                QMessageBox.warning(self, "Cannot Delete", "Cannot delete an offline file.")
                return  # Sale del metodo si el archivo esta offline
...
class StartupWindow(QWidget):
    ...
    def updateProgressBar(self):
        # Actualizar el valor de la barra de progreso para crear una animacion
        current_value = self.progressBar.value()
        new_value = current_value + 1 if current_value < 100 else 0
