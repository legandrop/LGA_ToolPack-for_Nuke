from PySide2.QtWidgets import QApplication, QTableWidget, QTableWidgetItem, QVBoxLayout, QWidget, QPushButton, QToolButton, QSizePolicy
from PySide2.QtWidgets import QItemDelegate, QStyle, QMessageBox, QCheckBox, QLabel, QHBoxLayout, QSpinBox, QFrame, QMenu, QAction
from PySide2.QtGui import QBrush, QColor, QPalette, QMovie
...
class FileScanner(QWidget):
    def __init__(self, parent=None):
        ...
        self.non_sequence_extensions = ['.mov', '.psd', '.avi']
        self.initUI()
...
##### Metodos para el UI:
    def initUI(self):
        ...
        self.copy_button.setToolTip("Copy the selected media to a specific folder and update the corresponding Read node's path")
        ...
        left_buttons_layout.insertWidget(3, self.copy_button) 
        ...
        self.column_label = QLabel('Extras')
        ...
        self.level_label = QLabel('Project Folder Depth')
        self.level_spinbox = QSpinBox()
        self.level_spinbox.setToolTip('Set the depth of folders to scan in the project directory')
        ...
        self.refresh_button.setToolTip('Rescan the project directory and refresh the data')
        ...
        self.table.setHorizontalHeaderLabels(['Media', 'Read', 'Status', 'Folder_Delete', 'Sequence'])
        ...
        # Para el color del texto de footage cuando esta seleccionado
    ...
    # Ajusta el tamano de la ventana para mostrar todos los contenidos
        # Primero se ajustan las columnas al contenido
        self.table.resizeColumnsToContents()

        # Calcular el ancho de la ventana basado en el ancho de las columnas
        width = self.table.verticalHeader().width() + 4  # Pequeno ajuste para evitar scroll horizontal
    ...
    def refresh_data(self):
        ...
        for _ in range(self.level_spinbox.value()):
    ...
    # Cambia el color de todas las etiquetas QLabel en las filas seleccionadas    
        for index in selected.indexes():
            if index.column() == 0:
                label = self.table.cellWidget(index.row(), 0)
                if label:
                    label.setStyleSheet(f"QLabel {{ color: black; font-size: {self.font_size}pt;}}")

        # Restaurar el color para las etiquetas QLabel en las filas deseleccionadas
        for index in deselected.indexes():
            if index.column() == 0:
                label = self.table.cellWidget(index.row(), 0)
                if label:
                    # Cambiar el color del texto a gris (120, 120, 120) cuando se deselecciona
                    label.setStyleSheet(f"QLabel {{ color: rgb(200, 200, 200); font-size: {self.font_size}pt;}}")

    def get_color_for_level(self, level):
        # Define los colores por nivel aqui
        colors = {
            0: "#ffff66",  # Amarillo   T
            1: "#c56cf0",  # Lavanda    Proye
            2: "#ff6ba1",  # Rojo claro Grupo
            3: "#0088ff",  # Rojo coral Shot
            7: "#ff6f69",  # Azul       Task
            5: "#28b5b5",  # Cian       Subcarpeta task
            6: "#ff9a8a",  # Purpura
            4: "#ffd369",  # Amarillo mostaza
            8: "#035c00",  # Verde oscuro
            9: "#008ae0",  # Azul claro
            10: "#008097", # Azul verdoso
            11: "#007049", # Verde marino
    ...
    def change_footage_text_color(self, state):
        for row in range(self.table.rowCount()):
            ...
            if label:
                ...
                parts = original_text.split('/')  # Dividir usando la barra normal
                
                if state:
                    # Aplica los colores a cada parte de la ruta, excepto al nombre del archivo
                    colored_parts = ['<span style="color: white;">/</span>'.join(
                        f"<span style='color: {self.get_color_for_level(i)};'>{part}</span>"
                        for i, part in enumerate(parts[:-1])
                    )]
                    
                    # Aplica negrita y color blanco al nombre del archivo
                    ...
                    # Si el checkbox esta desmarcado, muestra solo el nombre del archivo en negrita y blanco
                    file_name = f"<b style='color: white;'>{parts[-1]}</b>"
                    # Junta la ruta sin colorear excepto el nombre del archivo
...
##### Botones de la izq:
    def go_to_read(self):
        ...
        if selected_items:
            ...
            if read_node_name:
                # Busca el nodo en Nuke y lo selecciona en el Node Graph
                ...
                if read_node:
                    ...
                    print(f"No se encontro el nodo Read: {read_node_name}")
...
##### Buesqueda de archivos:
            ...
            if unmatched_nodes:
                ...
                if is_sequence:
                    ...
                    if os.path.exists(directory):  # Verifica si la carpeta existe
                        ...
                        #print("no existe seq")
                        # Marcar todos los archivos en la secuencia como offline
                        frame_range = "[Unknown]"  # O cualquier indicador de rango de cuadros desconocido
                        is_folder_deletable = False  # No se puede borrar una carpeta que no existe
    ...
    # Obtiene los archivos de los nodos Read en el script de Nuke
        ...
        if self.current_read_node_name:
            ...
            if read_node:
                ...
                if read_node is not None and read_node.Class() == 'Read':
                    # Establece la nueva ruta del archivo
                    read_node['file'].setValue(new_file_path)

            self.loading_window.stop()
            self.loading_window.stop()
            self.refresh_data()  # Actualizar la tabla al finalizar la copia            
...
# Clase principal que hereda de QWidget para crear la interfaz de usuario
    def paint(self, painter, option, index):
        # Modificar el color del texto para todos los estados
        if index.column() == 0:  # Aplicar solo en la columna 'Footage'
            # Configurar el color del texto como transparente
        ...
        if option.state & QStyle.State_Selected:
            option.palette.setColor(QPalette.Highlight, QColor(128, 128, 128))  # Un gris para la seleccion
