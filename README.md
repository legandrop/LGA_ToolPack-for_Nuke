...
## <img src="Doc_Media/image7.png" alt="" width="6" height="16" style="margin-right:3px;"> Media manager v2.51 | Lega
...
- <strong>Collect:</strong> Copia el script y toda la media que usa a una carpeta nueva, con las rutas relativas a esa carpeta. Es el único botón que no trabaja sobre la selección sino sobre el script entero, y por eso va aparte y sin atajo. Adentro del destino arma una subcarpeta por scan location —<em>input</em>, <em>assets</em>, <em>prerenders</em>, <em>publish</em>—, una <em>shot</em> para lo que está adentro del shot pero fuera de toda location, y una <em>outside</em> para lo que vive afuera; sólo se crean las que tienen algo. Los Write se reapuntan pero no se copian, porque su archivo es una salida. El <code>dataDirectory</code> de un CopyCat se recrea vacío: adentro viven los contact sheets del entrenamiento y los checkpoints alternativos, que son historia del proceso y no material del comp; el <code>.cat</code> que el nodo usa y el que evalúa un Inference sí se copian. Los knobs con expresiones TCL no se tocan, porque una expresión armada sobre <code>nuke.script_directory()</code> ya sigue sola al script. Al final guarda el <code>.nk</code> en la raíz del destino, así que conviene tener el script guardado antes de empezar. Si algo falla vuelve atrás en vez de dejar el script a medias: cancelar la copia no reescribe ningún path, un archivo que no se pudo copiar deja el suyo apuntando al original, y si el guardado falla se deshace la reescritura entera. <em>Salvedad:</em> correr Collect sobre un script ya colectado no reconoce las scan locations, porque en el destino se llaman <em>input</em>, <em>assets</em> y no como están escritas en los Settings; todo cae en <em>shot</em> u <em>outside</em>.
<br><br>
