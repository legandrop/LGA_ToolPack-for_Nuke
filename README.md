<p>
  ...
  <span style="font-style:italic;line-height:1;">Lega | v2.64</span><br>
</p>
<br clear="left">
...
- <strong>Relink:</strong> (Alt+L) Abre una ventana para elegir una ubicación para buscar un archivo que está marcado como offline. Busca en las carpeta y subcarpetas hasta encontrar un match, y cambia la ruta del Read por la ruta encontrada. El browser abre en la <strong>última carpeta que usaste</strong>, aunque haya sido en otra sesión; si esa carpeta ya no existe sube de a un nivel hasta la primera que exista.
...
- <strong>Reload All Reads:</strong> Abajo a la derecha, a la izquierda de <em>Rescan</em>. Le dice a Nuke que relea de disco todos los nodos que leen archivos —cualquiera que tenga el botón <em>Reload</em> en sus propiedades, no sólo los Read— y después vuelve a escanear. Sirve para cuando terminó un render y el Read sigue mostrando el cache viejo, o los frames que faltaban ya están y el nodo los sigue dando por ausentes.
- <strong>Collect:</strong> Copia el script y toda la media que usa a una carpeta nueva, y <strong>reproduce ahí la estructura del shot</strong>: las mismas carpetas que definen el Shot folder y las Scan locations de los Settings, con el <code>.nk</code> en su misma posición relativa. Por eso el script colectado resuelve su propio shot y sus propias locations, y abrir el Media Manager sobre él lee el collect y no el shot original. Es el único botón que no trabaja sobre la selección sino sobre el script entero, y por eso va aparte y sin atajo.<br>
Elegís la <strong>carpeta contenedora</strong> y Collect crea adentro la carpeta con el nombre del shot; si la que elegiste ya se llama así, la usa tal cual. El browser abre en la <strong>última carpeta donde colectaste</strong>, aunque haya sido en otra sesión; si ya no existe abre en la carpeta del <code>.nk</code> —a diferencia de Relink, no sube de nivel, porque subir desde una entrega vieja lleva a las entregas de otro proyecto—. Se crean todas las carpetas del shot, tengan contenido o no, para que el resultado sea un shot completo. Lo que vive <em>fuera</em> del shot va a la location <em>Input</em>, conservando su carpeta de origen; el cartel de confirmación dice cuál es antes de que aceptes. Si el Shot folder está apagado no hay estructura que reproducir, y ahí sí agrupa por nombre de location y lo avisa.<br>
Los Write se reapuntan pero no se copian, porque su archivo es una salida. El <code>dataDirectory</code> de un CopyCat se recrea vacío: adentro viven los contact sheets del entrenamiento y los checkpoints alternativos, que son historia del proceso y no material del comp; el <code>.cat</code> que el nodo usa y el que evalúa un Inference sí se copian. Los knobs con expresiones TCL no se tocan, porque una expresión armada sobre <code>nuke.script_directory()</code> ya sigue sola al script.<br>
Conviene tener el script guardado antes de empezar. Si algo falla vuelve atrás en vez de dejar el script a medias: cancelar la copia no reescribe ningún path, un archivo que no se pudo copiar deja el suyo apuntando al original, y si el guardado falla se deshace la reescritura entera. Lo único que rechaza es un destino donde el script colectado caería sobre el original.
<br><br>
...
Debajo de los botones, las pastillas cuentan cuántos archivos hay de cada estado sobre el total, sin que las afecte el buscador. A la derecha, separado por un divisor, se muestra el <strong>espacio libre del disco donde vive el script</strong> —por ejemplo <code>700 GB free on N:</code>, con el tamaño y la letra del disco resaltados—, que es a dónde va a parar todo lo que se copie con Copy to o con Collect. Lo acompaña un punto de color con el mismo lenguaje que las pastillas: <strong>verde</strong> de 200 GB para arriba, <strong>amarillo</strong> entre 100 y 200, y <strong>rojo</strong> por debajo de 100. Si ese dato no se puede leer, la etiqueta y su punto simplemente no aparecen.
<br><br>
